...
from ..common import Time, CardCipher
...
class PASELIHandler(Base):
    ...
    async def handle_eacoin_request(self, request: Node) -> Optional[Node]:
        """
        ...
        if method == 'checkin':
            ...
            userid = await self.data.local.user.from_cardid(cardid)
            if userid is None:
            ...
            valid = await self.data.local.user.validate_pin(userid, pin)
            if not valid:
                # Refuse to do anything
            ...
            session = await self.data.local.user.create_session(userid)
        ...
        if method == 'opcheckin':
            ...
            arcade = await self.data.local.machine.get_arcade(self.config['machine']['arcade'])
            if arcade is None:
                # Refuse to do anything
            ...
            session = await self.data.local.machine.create_session(arcade.id)
            root.add_child(Node.string('sessid', session))
            return root
            ...
            userid = await self.data.local.user.from_session(session)
            if userid is None:
                # Refuse to do anything
            ...
            userid = await self.data.local.user.from_session(sessid)
            if userid is None:
                arcadeid = await self.data.local.machine.from_session(sessid)
            else:
                arcadeid = None
            ...
            events = await self.data.local.network.get_events(
                userid=userid,
                arcadeid=arcadeid,
            ...
            if logtype == 'detail':
                ...
                for event in events:
                    ...
                    if event.userid is not None:
                        user = await self.data.local.user.get_user(event.userid)
                        if user is not None:
                            cards = await self.data.local.user.get_cards(user.id)
                            if len(cards) > 0:
            ...
            arcade.pin = newpass
            await self.data.local.machine.put_arcade(arcade)
            return root
            ...
            if session is not None:
                ...
                await self.data.local.user.destroy_session(session)
            ...
            if session is not None:
                # Destroy the session so it can't be used for any other purchases
                await self.data.local.machine.destroy_session(session)
