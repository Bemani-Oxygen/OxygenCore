...
class CardManagerHandler(Base):
    ...
    async def handle_cardmng_request(self, request: Node) -> Optional[Node]:
        """
        ...
        if method == 'inquire':
            ...
            bound = await self.has_profile(userid)
            expired = False
            if bound is False:
                if modelstring is not None:
                    ...
                    if oldgame is None:
                        ...
                        bound = await oldgame.has_profile(userid)
                        expired = True

            refid = await self.data.local.user.get_refid(self.game, self.version, userid)
            ...
            userid = await self.data.local.user.create_account(cardid, pin)
            if userid is None:
            ...
            refid = await self.data.local.user.create_refid(self.game, self.version, userid)
            root = Node.void('cardmng')
            root.set_attribute('dataid', refid)
            root.set_attribute('refid', refid)
            return root
