...
from ...common import ValidatedDict, Time
from .base import BaseData, metadata
from ..types import User, Achievement, Link, UserID, ArcadeID
...
class UserData(BaseData):
    ...
    async def from_cardid(self, cardid: str) -> Optional[UserID]:
        """
        ...
        cursor = await self.execute(sql, {'id': cardid})
    ...
    async def from_refid(self, game: str, version: int, refid: str) -> Optional[UserID]:
        """
        ...
        cursor = await self.execute(sql, {'game': game, 'version': version, 'refid': refid})
    ...
    async def from_extid(self, game: str, version: int, extid: int) -> Optional[UserID]:
        """
        ...
        cursor = await self.execute(sql, {'game': game, 'extid': extid})
    ...
    async def from_session(self, session: str) -> Optional[UserID]:
        """
        ...
        """
        userid = await self._from_session(session, 'userid')
        if userid is None:
            return None
        return UserID(userid)

    async def get_all_cards(self) -> List[Tuple[str, UserID]]:
        """
    ...
    async def get_cards(self, userid: UserID) -> List[str]:
        """
        ...
        cursor = await self.execute(sql, {'userid': userid})
        return [str(res['id']).upper() for res in cursor.fetchall()]

    async def add_card(self, userid: UserID, cardid: str) -> None:
        """
        ...
        await self.execute(sql, {'userid': userid, 'cardid': cardid})

    async def destroy_card(self, userid: UserID, cardid: str) -> None:
        """
        ...
        await self.execute(sql, {'cardid': cardid, 'userid': userid})

    async def validate_pin(self, userid: UserID, pin: str) -> bool:
        """
    ...
    async def update_pin(self, userid: UserID, pin: str) -> None:
        """
        ...
        await self.execute(sql, {'pin': pin, 'userid': userid})

    async def get_profile(self, game: str, version: int, userid: UserID) -> Optional[ValidatedDict]:
        """
        ...
        cursor = await self.execute(sql, {'userid': userid, 'game': game, 'version': version})
        ...
        cursor = await self.execute(sql, {'refid': profile['refid']})
    ...
    async def get_any_profile(self, game: str, version: int, userid: UserID) -> Optional[ValidatedDict]:
        """
        ...
        Returns:
            A dictionary previously stored by a game class if found, or None otherwise.
        """
        played = await self.get_games_played(userid)
        versions = [p[1] for p in played if p[0] == game]
        versions.sort(reverse=True)

        if version in versions:
            return await self.get_profile(game, version, userid)
        elif len(versions) > 0:
            return await self.get_profile(game, versions[0], userid)
        else:
            return None

    async def get_any_profiles(self, game: str, version: int, userids: List[UserID]) -> List[Tuple[UserID, Optional[ValidatedDict]]]:
        """
        ...
        """
        return [
            (userid, await self.get_any_profile(game, version, userid))
            for userid in userids
        ]

    async def get_games_played(self, userid: UserID) -> List[Tuple[str, int]]:
        """
    ...
    async def get_all_profiles(self, game: str, version: int) -> List[Tuple[UserID, ValidatedDict]]:
        """
        ...
        cursor = await self.execute(sql, {'game': game, 'version': version})
    ...
    async def get_all_players(self, game: str, version: int) -> List[UserID]:
        """
    ...
    async def get_all_achievements(self, game: str, version: int) -> List[Tuple[UserID, Achievement]]:
        """
    ...
    async def put_profile(self, game: str, version: int, userid: UserID, profile: Dict[str, Any]) -> None:
        """
        ...
        """
        refid = await self.get_refid(game, version, userid)
        ...
        sql = (
                ...
                "VALUES (:refid, :json) " +
                "ON CONFLICT(refid) DO UPDATE SET data=excluded.data"
        )
        await self.execute(sql, {'refid': refid, 'json': self.serialize(profile)})

    async def get_achievement(self, game: str, version: int, userid: UserID, achievementid: int, achievementtype: str) -> Optional[ValidatedDict]:
        """
        ...
        cursor = await self.execute(sql, {'refid': refid, 'id': achievementid, 'type': achievementtype})
    ...
    async def get_achievements(self, game: str, version: int, userid: UserID) -> List[Achievement]:
        """
        ...
        cursor = await self.execute(sql, {'refid': refid})
    ...
    async def put_achievement(self, game: str, version: int, userid: UserID, achievementid: int, achievementtype: str, data: Dict[str, Any]) -> None:
        """
        ...
        sql = (
                ...
                "ON CONFLICT(refid, id, type) DO UPDATE SET data=excluded.data"
        )
        await self.execute(sql, {'refid': refid, 'id': achievementid, 'type': achievementtype, 'data': self.serialize(data)})

    async def destroy_achievement(self, game: str, version: int, userid: UserID, achievementid: int, achievementtype: str) -> None:
        """
        ...
        await self.execute(sql, {'refid': refid, 'id': achievementid, 'type': achievementtype})

    async def get_time_based_achievements(
            self,
            game: str,
            version: int,
            userid: UserID,
        ...
        cursor = await self.execute(sql, {'refid': refid, 'type': achievementtype, 'since': since, 'until': until})
    ...
    async def put_time_based_achievement(
            self,
            game: str,
            version: int,
            userid: UserID,
        ...
        await self.execute(sql, {'refid': refid, 'id': achievementid, 'type': achievementtype, 'ts': Time.now(), 'data': self.serialize(data)})

    async def get_all_time_based_achievements(self, game: str, version: int) -> List[Tuple[UserID, Achievement]]:
        """
    ...
    async def get_refid(self, game: str, version: int, userid: UserID) -> str:
        """
        ...
        if result is not None:
            ...
            return await self.create_refid(game, version, userid)

    async def get_extid(self, game: str, version: int, userid: UserID) -> int:
        """
        ...
        async def fetch_extid() -> Optional[int]:
            sql = "SELECT extid FROM extid WHERE userid = :userid AND game = :game"
            cursor = await self.execute(sql, {'userid': userid, 'game': game})
        ...
        if extid is not None:
            ...
            await self.create_refid(game, version, userid)
            extid = fetch_extid()
            if extid is not None:
                return extid
            else:
    ...
    async def create_session(self, userid: UserID, expiration: int = (30 * 86400)) -> str:
        """
        ...
        """
        return await self._create_session(userid, 'userid', expiration)
    ...
    async def destroy_session(self, session: str) -> None:
        ...
        """
        await self._destroy_session(session, 'userid')

    async def create_refid(self, game: str, version: int, userid: UserID) -> str:
        """
        ...
        while True:
            ...
            cursor = await self.execute(sql, {'extid': extid})
        ...
        try:
            cursor = await self.execute(sql, {'game': game, 'extid': extid, 'userid': userid})
        ...
        try:
            cursor = await self.execute(sql, {'game': game, 'version': version, 'refid': refid, 'userid': userid})
    ...
    async def create_account(self, cardid: str, pin: str) -> Optional[UserID]:
        """
        ...
        sql = "INSERT INTO user (pin) VALUES (:pin)"
        cursor = await self.execute(sql, {'pin': pin})
        if cursor.rowcount != 1:
            return None
        ...
        cursor = await self.execute(sql, {'cardid': cardid, 'userid': userid})
        if cursor.rowcount != 1:
            return None
