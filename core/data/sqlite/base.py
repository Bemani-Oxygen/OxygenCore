...
class BaseData:
    ...
    async def execute(self, sql: str, params: Optional[Dict[str, Any]] = None, safe_write_operation: bool = False) -> CursorResult:
        """
    ...
    async def _from_session(self, session: str, sesstype: str) -> Optional[int]:
        """
        ...
        cursor = await self.execute(sql, {'session': session, 'type': sesstype, 'timestamp': Time.now()})
    ...
    async def _create_session(self, opid: int, optype: str, expiration: int = (30 * 86400)) -> str:
        """
        ...
        while True:
            ...
            cursor = await self.execute(sql, {'session': session})
    ...
    async def _destroy_session(self, session: str, sesstype: str) -> None:
        """
        ...
        await self.execute(sql, {'session': session, 'sesstype': sesstype}, safe_write_operation=True)

        # Also weed out any other defunct sessions
        sql = "DELETE FROM session WHERE expiration < :timestamp"
        await self.execute(sql, {'timestamp': Time.now()}, safe_write_operation=True)
