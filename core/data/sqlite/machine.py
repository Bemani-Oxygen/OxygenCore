...
from ...common import ValidatedDict
from .base import BaseData, metadata
from ..types import Machine, Arcade, UserID, ArcadeID
...
class MachineData(BaseData):
    ...
    async def from_port(self, port: int) -> Optional[str]:
        """
        ...
        cursor = await self.execute(sql, {'port': port})
    ...
    async def from_machine_id(self, machine_id: int) -> Optional[str]:
        """
        ...
        cursor = await self.execute(sql, {'id': machine_id})
    ...
    async def from_userid(self, userid: UserID) -> List[ArcadeID]:
        """
    ...
    async def from_session(self, session: str) -> Optional[ArcadeID]:
        """
        ...
        """
        arcadeid = await self._from_session(session, 'arcadeid')
        if arcadeid is None:
            return None
        return ArcadeID(arcadeid)

    async def get_machine(self, pcbid: str) -> Optional[Machine]:
        """
        ...
        cursor = await self.execute(sql, {'pcbid': pcbid})
    ...
    async def get_all_machines(self, arcade: Optional[ArcadeID] = None) -> List[Machine]:
        """
        ...
        cursor = await self.execute(sql, data)
        return [
            Machine(
                result['id'],
                result['pcbid'],
    ...
    async def put_machine(self, machine: Machine) -> None:
        """
    ...
    async def create_machine(self, pcbid: str, name: str = 'Oxygen', description: str = '', arcade: Optional[ArcadeID] = None) -> Machine:
        """
        ...
        while True:
            ...
            try:
                ...
                await self.execute(sql, {'pcbid': pcbid, 'name': name, 'description': description, 'port': port, 'arcadeid': arcade})
            except Exception:
                # Failed to add machine, try with new port
                continue

            machine = await self.get_machine(pcbid)
            if machine is not None:
                return machine

    async def destroy_machine(self, pcbid: str) -> None:
        """
        ...
        await self.execute(sql, {'pcbid': pcbid})

    async def create_arcade(self, name: str, description: str, data: Dict[str, Any], owners: List[UserID]) -> Arcade:
        """
        ...
        for owner in owners:
            ...
            await self.execute(sql, {'userid': owner, 'arcadeid': arcadeid})
        new_arcade = await self.get_arcade(arcadeid)
        if new_arcade is None:
            raise Exception("Failed to create an arcade!")
        return new_arcade

    async def get_arcade(self, arcadeid: ArcadeID) -> Optional[Arcade]:
        """
        ...
        cursor = await self.execute(sql, {'id': arcadeid})
    ...
    async def put_arcade(self, arcade: Arcade) -> None:
        """
        ...
        await self.execute(sql, {'arcadeid': arcade.id})
        for owner in arcade.owners:
            sql = "INSERT INTO arcade_owner (userid, arcadeid) VALUES(:userid, :arcadeid)"
            await self.execute(sql, {'userid': owner, 'arcadeid': arcade.id})

    async def destroy_arcade(self, arcadeid: ArcadeID) -> None:
        """
        ...
        await self.execute(sql, {'arcadeid': arcadeid})
        sql = "DELETE FROM `arcade_owner` WHERE arcadeid = :arcadeid"
        await self.execute(sql, {'arcadeid': arcadeid})
        sql = "UPDATE `machine` SET arcadeid = NULL WHERE arcadeid = :arcadeid"
        await self.execute(sql, {'arcadeid': arcadeid})

    async def get_all_arcades(self) -> List[Arcade]:
        """
    ...
    async def get_settings(self, arcadeid: ArcadeID, game: str, version: int, setting: str) -> Optional[ValidatedDict]:
        """
        ...
        cursor = await self.execute(sql, {'id': arcadeid, 'game': game, 'version': version, 'type': setting})
    ...
    async def put_settings(self, arcadeid: ArcadeID, game: str, version: int, setting: str, data: Dict[str, Any]) -> None:
        """
        ...
        sql = (
                ...
                "ON CONFLICT(arcadeid, game, version, type) DO UPDATE SET data=excluded.data"
        )
        await self.execute(sql, {'id': arcadeid, 'game': game, 'version': version, 'type': setting, 'data': self.serialize(data)})

    async def create_session(self, arcadeid: ArcadeID, expiration: int = (30 * 86400)) -> str:
        """
        ...
        """
        return await self._create_session(arcadeid, 'arcadeid', expiration)
    ...
    async def destroy_session(self, session: str) -> None:
        ...
        """
        await self._destroy_session(session, 'arcadeid')
